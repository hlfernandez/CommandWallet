...
class CommandWallet:
    ...
    def create_right_panel(self, parent):
        ...
        # Buttons frame (Run Command, Export Cron, and Config)
        ...
        ctk.CTkButton(buttons_frame, text="Export Cron", command=self.show_cron_dialog).pack(side="left", padx=(5, 5), pady=10)
        
        ctk.CTkButton(buttons_frame, text="Config", command=self.show_config_dialog).pack(side="left", padx=(5, 10), pady=10)
    ...
    def show_cron_dialog(self):
        """Show cron configuration dialog"""
        if not self.current_command_id:
            messagebox.showwarning("No Command", "Please select a command to export to cron.")
            return
        
        cron_window = ctk.CTkToplevel(self.root)
        cron_window.title("Export to Cron")
        cron_window.geometry("700x600")
        cron_window.transient(self.root)
        
        # Wait for window to be fully created before grabbing focus
        cron_window.after(100, lambda: cron_window.grab_set())
        
        # Title
        ctk.CTkLabel(cron_window, text="Export Command to Cron", 
        ...
        info_frame = ctk.CTkFrame(cron_window)
        info_frame.pack(fill="x", padx=20, pady=(0, 20))
        
        ctk.CTkLabel(info_frame, text=f"Command: {command_name}", 
                     font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=10, pady=(10, 5))
        ctk.CTkLabel(info_frame, text=f"Script: {command_text}", 
                     font=ctk.CTkFont(size=12), wraplength=650).pack(anchor="w", padx=10, pady=(0, 10))
        
        # Cron fields frame
        fields_frame = ctk.CTkFrame(cron_window)
        fields_frame.pack(fill="x", padx=20, pady=(0, 20))
        
        ctk.CTkLabel(fields_frame, text="Cron Schedule Configuration", 
        ...
        for i, (label, range_text, desc) in enumerate(field_info):
            # Label
            ctk.CTkLabel(grid_frame, text=label, font=ctk.CTkFont(size=12, weight="bold")).grid(
                row=i, column=0, sticky="w", padx=(10, 20), pady=5)
            
            # Entry
            entry = ctk.CTkEntry(grid_frame, width=80, placeholder_text="*")
            entry.grid(row=i, column=1, padx=(0, 20), pady=5)
            entry.insert(0, "*")
            entry.bind('<KeyRelease>', self.update_cron_preview)
            self.cron_entries.append(entry)
            
            # Range
            ctk.CTkLabel(grid_frame, text=f"({range_text})", 
                         font=ctk.CTkFont(size=10)).grid(row=i, column=2, sticky="w", padx=(0, 20), pady=5)
            
            # Description
            ctk.CTkLabel(grid_frame, text=desc, font=ctk.CTkFont(size=10), 
                         text_color="gray").grid(row=i, column=3, sticky="w", padx=(0, 10), pady=5)
        
        # Preview frame
        preview_frame = ctk.CTkFrame(cron_window)
        preview_frame.pack(fill="x", padx=20, pady=(0, 20))
        
        ctk.CTkLabel(preview_frame, text="Schedule Preview", 
                     font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=10, pady=(10, 5))
        
        self.cron_preview_label = ctk.CTkLabel(preview_frame, text="Every minute", 
                                               font=ctk.CTkFont(size=12), wraplength=650)
        self.cron_preview_label.pack(anchor="w", padx=10, pady=(0, 10))
        
        # Generated cron entry frame
        output_frame = ctk.CTkFrame(cron_window)
        output_frame.pack(fill="both", expand=True, padx=20, pady=(0, 20))
        
        ctk.CTkLabel(output_frame, text="Generated Cron Entry", 
                     font=ctk.CTkFont(size=14, weight="bold")).pack(anchor="w", padx=10, pady=(10, 5))
        
        # Text area for cron entry
        self.cron_output_text = ctk.CTkTextbox(output_frame, height=120, 
                                               font=ctk.CTkFont(family="Consolas", size=11))
        self.cron_output_text.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        
        # Buttons frame
        buttons_frame = ctk.CTkFrame(cron_window)
        buttons_frame.pack(fill="x", padx=20, pady=(0, 20))
        
        def copy_cron_entry():
            cron_text = self.cron_output_text.get("1.0", "end-1c")
            if cron_text.strip():
                cron_window.clipboard_clear()
                cron_window.clipboard_append(cron_text)
                cron_window.update()
                # Show brief confirmation
                copy_btn.configure(text="Copied!")
                cron_window.after(1500, lambda: copy_btn.configure(text="Copy to Clipboard"))
        
        def close_cron_dialog():
            cron_window.destroy()
        
        # Control buttons
        copy_btn = ctk.CTkButton(buttons_frame, text="Copy to Clipboard", command=copy_cron_entry)
        copy_btn.pack(side="right", padx=(5, 10), pady=10)
        
        ctk.CTkButton(buttons_frame, text="Close", command=close_cron_dialog, 
                      fg_color="gray", hover_color="darkgray").pack(side="right", padx=(5, 5), pady=10)
        
        # Initialize preview
        self.update_cron_preview()
        
        # Focus on first entry field
        cron_window.after(200, lambda: self.cron_entries[0].focus())
    
    def update_cron_preview(self, event=None):
        """Update cron schedule preview and generated entry"""
        if not hasattr(self, 'cron_entries'):
            return
        ...
        description = self.get_cron_description(minute, hour, day, month, weekday)
